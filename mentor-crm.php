...
function install_mentor_crm(){
		...
		$upload_dir = $upload_dir . '/mentor-crm-private';
...
function mentor_screen_initial() {
	...
	if (!empty($_GET['delete-lid'])) {
	    $wpdb->delete($wpdb->prefix.'mentor_leads', array( 'LID' => $_GET['delete-lid'] ) );
	    $wpdb->delete($wpdb->prefix.'mentor_orders', array( 'LID' => $_GET['delete-lid'] ) );
	    $upload = wp_upload_dir();
	    $upload_dir = $upload['basedir'];
	    $lead_dir = $upload_dir . '/mentor-crm-private/'.$_GET['delete-lid'];
	    if (is_dir($lead_dir)) {
	    	MentorRemoveDir($lead_dir);
	    }
	    $notices .='<div class="notice notice-error is-dismissible">
	                    <p>Lead #'.$_GET['delete-lid'].' Eliminado!</p>
	                </div>';
	}
...
function mentor_lead_capture() {
	...
	$msg_fail = false;
	$max_image_file_size = 5*1024*1024; // 4 is equal to 5mb
	...
	if(isset($_FILES)){
	  ...
	  foreach ($_FILES['lead_image']['tmp_name'] as $key => $lead_image) {
	    if( $_FILES['lead_image']['error'][$key] === UPLOAD_ERR_OK ) {
	      ...
	      // Check file size
	      if ($_FILES["lead_image"]["size"][$key] > $max_image_file_size) {
	         $msg_fail .= 'Lo sentimos, tu archivo '.($key + 1).' es demasiado grande. / Sorry, your file '.($key + 1).' is too large.';
	         $upload_ok = false;
	      }
	      // Check extension
	      if(!in_array($imageFileType,$extensions_arr) ){
	          $msg_fail .= 'Lo sentimos, tu archivo '.($key + 1).' no es válido. / Sorry, your file '.($key + 1).' is not valid.';
	          $upload_ok = false;
	      }
	      if ($upload_ok) {
	          move_uploaded_file($_FILES['lead_image']['tmp_name'][$key], trailingslashit($upload_dir).'SecureFile'.$key.'.'.$imageFileType);
	      }
	    }
	  }
	}
	$return = array('msg'=>'ok','payment_url'=>$payment_url,'lid'=>$LID,'msg_fail'=>$msg_fail);
	wp_send_json($return);
}

add_action( 'wp_ajax_mentor_lead_capture_2fire', 'mentor_lead_capture_2fire' );
add_action( 'wp_ajax_nopriv_mentor_lead_capture_2fire', 'mentor_lead_capture_2fire' );
function mentor_lead_capture_2fire() {
	global $wpdb,$crmcountries;
	$LID = $_POST['lid'];
	$register_date = date('d/m/Y h:i a');
	$lead = $wpdb->get_row("SELECT * FROM {$wpdb->prefix}mentor_leads WHERE LID = {$LID}");
	$extras = array();
	$order_reference = $wpdb->get_var("SELECT reference FROM {$wpdb->prefix}mentor_orders WHERE LID = {$LID}");
	$payment_url = trailingslashit(home_url()).'mentor-crm-payment/'.$order_reference;
	$fullname = $lead->fullname;
	$email = $lead->email;
	$reason = $lead->reason;
	$phone = $lead->phone;
	...
	$city = $lead->city;
	//$time = date('H:i:s',strtotime($lead->time));
	$date = date('d/m/Y',strtotime($lead->date));
	$tipo_cita = array(1=>'TELE ORIENTACIÓN MÉDICA',2=>'PRESENCIAL');
	$tipo_cita = $tipo_cita[$lead->process];
	$country = $crmcountries[$country];
	// notify to user email
	ob_start();
	include MENTOR_CRM_PATH.'/inc/emails/welcome-lead.php';
	$welcome = ob_get_clean();
	mentor_email($email,get_option('mentor_crm_cliente_name').' - Solicitud #'.$LID,$welcome,$extras,$LID);
	// notify to admins emails
	ob_start();
	include MENTOR_CRM_PATH.'/inc/emails/admin-new-lead.php';
	$new_lead = ob_get_clean();
	mentor_email(get_option('mentor_crm_admin_notify'),get_option('mentor_crm_cliente_name').' CRM - Nuevo Lead: '.$fullname.' - #'.$LID,$new_lead,$extras,$LID);
	$return = array('msg'=>'ok');
	wp_send_json($return);
}